"""Create an isolated localhost-only PostgreSQL database for Compliance integration tests.
...
async def main():
    ...
    name = f"aegis_compliance_test_{int(time.time())}"
    try:
        for role in (
            "anon",
            "authenticated",
    ...
    try:
        ...
        await db.execute(
            "CREATE SCHEMA procurement; CREATE SCHEMA crm; CREATE SCHEMA fleet;"
        )
        for source, target in [
            ("supplier_records", "procurement.suppliers"),
            ("fleet", "fleet.fleet"),
        ]:
            sql = create_table("001_imperium_foundation.sql", source)
            sql = sql.replace("CREATE TABLE " + source, "CREATE TABLE " + target)
            sql = (
                sql.replace("REFERENCES organizations", "REFERENCES core.organizations")
                .replace("REFERENCES users", "REFERENCES core.users")
                .replace("REFERENCES projects", "REFERENCES projects.projects")
            )
            await db.execute(sql)
        await db.execute(create_table("002_imperium_schemas.sql", "crm.subcontractors"))
        async with db.transaction():
            await db.execute(
                (ROOT / "migrations/177_compliance_foundation.sql").read_text()
            )
        path = ROOT / ".aegis-runtime/compliance-test/database-name.txt"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(name)
        print(
            f"Isolated database created: {name}; migrations 176 and 177 applied successfully."
        )
    finally:
        await db.close()
