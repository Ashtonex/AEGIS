...
`run_stale_quote_check.py` and `run_weekly_digest.py` are not run through
...
Both scripts fire a named automation trigger (`quotation_stale`,
`weekly_digest`) through the same `crm.automation_rules` engine used
everywhere else in the CRM - so what actually happens (email, in-app
notification, etc.) is configured via automation rules, not hardcoded in
