...
class PublicIntakeContractTests(unittest.TestCase):
    ...
    def test_client_submissions_include_idempotency_keys(self) -> None:
        source = (WEB / "lib" / "api.ts").read_text(encoding="utf-8") + "".join(
    sorted_p.read_text(encoding="utf-8")
    for sorted_p in [(WEB / "lib" / "api") / n for n in ("core.ts", "website.ts", "crm.ts", "procurement.ts", "fleet.ts", "finance.ts", "inventory.ts", "hr.ts", "compliance.ts", "documents.ts", "reports.ts", "quotations.ts", "banking.ts", "data-room.ts")]
)
        for function in (
