...
def _line_filter_sql(org_id: str, filters: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if filters.get("unallocated_cash"):
        # withdrawals whose cash hasn't been fully accounted for yet
        clauses.append(
            "bsl.category = 'cash_withdrawal' AND bsl.amount < 0 AND -bsl.amount > "
            "(SELECT COALESCE(sum(a.amount), 0) FROM finance.bank_line_allocations a WHERE a.line_id = bsl.id)"
        )
    ...
    rows = await db.execute(
        text(f"""
            SELECT bsl.id, bsl.import_id, bsl.cash_account_id, bsl.line_number, bsl.transaction_date,
                   ...
                   (SELECT COALESCE(sum(a.amount), 0) FROM finance.bank_line_allocations a
                    WHERE a.line_id = bsl.id) AS allocated_amount
            FROM finance.bank_statement_lines bsl
...
async def tag_lines(
    ...
    line_ids = [r.id for r in result]
    return {"updated": len(line_ids), "line_ids": line_ids}
