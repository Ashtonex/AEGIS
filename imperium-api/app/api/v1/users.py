...
from app.api.responses import StandardResponse, create_success_response, create_error_response
...
async def get_users(db: DBSessionDep, skip: int = 0, limit: int = 100):
    ...
    return create_success_response(data=[{"id": str(u.id), "email": u.email, "full_name": u.full_name} for u in users])
