...
"""
Module: equipment_assets
Description: Auto-generated CRUD endpoints for fleet.equipment_assets.
"""
...
async def list_items(
    ...
    _: dict = Depends(require_permission("equipment_assets.read")),
):
    # Fetch active records scoped to the user's organization
    query = text("""
        SELECT *
        FROM fleet.equipment_assets
    ...
    return {
        "success": True,
        "data": items,
        "message": "equipment_assets listed.",
...
async def create_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("equipment_assets.create")),
):
    ...
    query = insert_returning_id_sql("fleet.equipment_assets", safe_keys, safe_keys)
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"id": new_id},
            "message": "equipment_assets created.",
            "meta": {},
        }
...
async def get_item(
    ...
    return {
        ...
        "message": "equipment_assets retrieved.",
        "meta": {},
    }
...
async def update_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("equipment_assets.update")),
):
    payload = await request.json()
    safe_keys = safe_payload_columns(payload.keys())
    ...
    query = update_returning_id_sql("fleet.equipment_assets", safe_keys, safe_keys)
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"id": item_id},
            "message": "equipment_assets updated.",
            "meta": {},
        }
...
async def delete_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("equipment_assets.delete")),
):
    query = text("""
        UPDATE fleet.equipment_assets
    ...
    return {
        "success": True,
        "data": None,
        "message": "equipment_assets deleted (soft delete).",
        "meta": {},
    }
