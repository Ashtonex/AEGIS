...
Description: Auto-generated CRUD endpoints for core.internal_messages.
"""
...
async def list_items(
    ...
    _: dict = Depends(require_permission("internal_messages.read")),
):
    # Fetch active records scoped to the user's organization
    query = text("""
        SELECT *
        FROM core.internal_messages
    ...
    return {
        "success": True,
        "data": items,
        "message": "internal_messages listed.",
...
async def create_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("internal_messages.create")),
):
    ...
    query = insert_returning_id_sql("core.internal_messages", safe_keys, safe_keys)
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"id": new_id},
            "message": "internal_messages created.",
            "meta": {},
        }
...
async def get_item(
    ...
    return {
        ...
        "message": "internal_messages retrieved.",
        "meta": {},
    }
...
async def update_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("internal_messages.update")),
):
    payload = await request.json()
    safe_keys = safe_payload_columns(payload.keys())
    ...
    query = update_returning_id_sql("core.internal_messages", safe_keys, safe_keys)
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"id": item_id},
            "message": "internal_messages updated.",
            "meta": {},
        }
...
async def delete_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("internal_messages.delete")),
):
    query = text("""
        UPDATE core.internal_messages
    ...
    return {
        "success": True,
        "data": None,
        "message": "internal_messages deleted (soft delete).",
        "meta": {},
    }
