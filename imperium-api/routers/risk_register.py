...
Description: Auto-generated CRUD endpoints for executive.risk_register.
"""
...
async def list_items(
    ...
    _: dict = Depends(require_permission("risk_register.read")),
):
    # Fetch active records scoped to the user's organization
    query = text("""
        SELECT *
        FROM executive.risk_register
...
async def create_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("risk_register.create")),
):
    ...
    query = insert_returning_id_sql("executive.risk_register", safe_keys, safe_keys)
...
async def get_item(
    ...
    return {
        ...
        "message": "risk_register retrieved.",
        "meta": {},
    }
...
async def update_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("risk_register.update")),
):
    payload = await request.json()
    safe_keys = safe_payload_columns(payload.keys())
    ...
    query = update_returning_id_sql("executive.risk_register", safe_keys, safe_keys)
...
async def delete_item(
    ...
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    _: dict = Depends(require_permission("risk_register.delete")),
):
    query = text("""
        UPDATE executive.risk_register
    ...
    return {
        "success": True,
        "data": None,
        "message": "risk_register deleted (soft delete).",
        "meta": {},
    }
