...
async def list_users(
    ...
    query = text(
        "SELECT id, email, full_name, is_active FROM core.users WHERE organization_id = :org_id AND is_deleted = false"
    )
...
async def get_user(
    ...
    query = text(
        "SELECT id, email, full_name, is_active FROM core.users WHERE id = :id AND organization_id = :org_id AND is_deleted = false"
    )
...
async def update_user(
    ...
    if payload.is_active is not None and str(id) == user["user_id"]:
        raise HTTPException(
            status_code=403, detail="Users cannot change their own activation status."
        )
...
async def delete_user(
    ...
    query = text(
        "UPDATE core.users SET is_deleted = true, updated_at = NOW() WHERE id = :id AND organization_id = :org_id"
    )
...
async def assign_role(
    ...
    query = text("""
        ...
        SELECT :user_id, :role_id, :org_id
        WHERE EXISTS (SELECT 1 FROM core.users WHERE id = :user_id AND organization_id = :org_id AND is_deleted = false)
          AND EXISTS (SELECT 1 FROM core.roles WHERE id = :role_id AND organization_id = :org_id AND is_deleted = false)
        ON CONFLICT (user_id, role_id) DO NOTHING
        RETURNING user_id
    """)
    result = await db.execute(
        query, {"user_id": id, "role_id": payload.role_id, "org_id": user["org_id"]}
    )
