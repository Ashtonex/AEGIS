CREATE TABLE IF NOT EXISTS projects.project_profiles (
    ...
    region VARCHAR(120), province VARCHAR(120), site_location VARCHAR(255),
    latitude NUMERIC(9, 6), longitude NUMERIC(9, 6),
    viability_status VARCHAR(50), viability_notes TEXT,
    delivery_manager_id UUID REFERENCES core.users(id),
    budget_amount NUMERIC(15, 2), forecast_cost NUMERIC(15, 2),
...
CREATE TABLE IF NOT EXISTS projects.project_checks (
    ...
    organization_id UUID NOT NULL REFERENCES core.organizations(id),
    check_name VARCHAR(255) NOT NULL, check_type VARCHAR(100), status VARCHAR(50) NOT NULL,
    completed_at TIMESTAMPTZ, evidence_reference TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
