CREATE TABLE IF NOT EXISTS hr.employee_profiles (
 user_id UUID PRIMARY KEY REFERENCES core.users(id) ON DELETE CASCADE, organization_id UUID NOT NULL REFERENCES core.organizations(id) ON DELETE CASCADE,
 preferred_name VARCHAR(100), first_name VARCHAR(100), last_name VARCHAR(100), work_phone VARCHAR(40), job_title VARCHAR(150), department VARCHAR(150), location VARCHAR(150), timezone VARCHAR(64), bio TEXT,
 linkedin_url TEXT, portfolio_url TEXT, website_url TEXT, avatar_path TEXT, skills JSONB NOT NULL DEFAULT '[]'::jsonb, languages JSONB NOT NULL DEFAULT '[]'::jsonb,
