...
export default function ComplianceFoundation() {
  ...
  async function prepare(kind: string) {
    ...
    try {
      const kinds = ["domains", "authorities", "sources", "obligation-versions", "applicability-assessments", "lookups/users", "lookups/project"];
  ...
  const obligationFields: Field[] = [
    ...
    {name:"requirement",label:"Required outcome",type:"textarea"},{name:"owner_id",label:"Accountable owner",choices:choices("lookups/users")},
  ...
  const fields: Field[] = form === "domains" || form === "authorities" ? [{name:"code",label:"Code"},{name:"name",label:"Name"},{name:"jurisdiction",label:"Jurisdiction"}]
    ...
    : form === "applicability-assessments" ? [{name:"obligation_version_id",label:"Active obligation version",choices:choices("obligation-versions")},{name:"subject_kind",label:"Subject kind",choices:["organisation","project","worker","supplier","subcontractor","asset"].map(v=>({value:v,label:v}))},{name:"subject_id",label:"Subject ID from the owning module"},{name:"project_id",label:"Project (required for project requirements)",optional:true,choices:choices("lookups/project")},{name:"outcome",label:"Proposed applicability",choices:[{value:"applicable",label:"Applicable"},{value:"not_applicable",label:"Not applicable — requires independent approval"}]},{name:"reason",label:"Assessment reason",type:"textarea"},{name:"basis",label:"Source-backed assessment basis",type:"textarea"},{name:"review_on",label:"Review date",type:"date"}]
    : form === "assignments" ? [{name:"assessment_id",label:"Approved applicable assessment",choices:choices("applicability-assessments")},{name:"owner_id",label:"Responsible owner",choices:choices("lookups/users")},{name:"reason",label:"Assignment reason",type:"textarea"}]
    : form === "reassign" ? [{name:"owner_id",label:"New accountable owner",choices:choices("lookups/users")},{name:"reason",label:"Reason for reassignment",type:"textarea"}]
    : [{name:"user_id",label:"User",choices:choices("lookups/users")},{name:"project_id",label:"Project",choices:choices("lookups/project")}];

  async function command(path: string, payload: object) {
    if (!navigator.onLine) throw new Error("Reconnect before submitting a compliance command. Your draft has not been submitted.");
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (form === "reassign" && selected) { path=`assignments/${selected.id}/reassign`; payload.expected_version=selected.version; }
  ...
  return <main className="mx-auto max-w-7xl space-y-5 p-3 text-slate-100 sm:p-6">
    ...
    {!online && <p role="alert" className="rounded border border-amber-700 p-3 text-amber-200">Offline: displayed records may be stale. Reconnect to submit or approve. Drafts remain unsent.</p>}
    ...
    {selected && <section className="space-y-3 rounded-xl border border-cyan-800 p-4"><div className="flex justify-between"><h2 className="text-lg font-semibold">Record detail</h2><button className={base} onClick={()=>{setSelected(null);setAction(null);}}>Close detail</button></div><dl className="grid gap-2 sm:grid-cols-2">{Object.entries(selected).filter(([k])=>!["organization_id","is_deleted"].includes(k)).map(([k,v])=><div key={k} className="min-w-0"><dt className="text-xs text-slate-400">{k.replaceAll("_"," ")}</dt><dd className="break-words text-sm">{typeof v==="object"?JSON.stringify(v):String(v??"Not recorded")}</dd></div>)}</dl>
      <div className="flex flex-wrap gap-2">
        {tab==="assignments" && can("assign") && <button className={base} onClick={()=>void prepare("reassign")}>Reassign owner</button>}
