...
// Only the active tab's panel ships to the browser instead of all 4 at once.
function PanelLoading() {
  return <Skeleton className="h-64 w-full" />;
}
const StockLevelsTab = dynamic(() => import("./InventoryTabPanels").then((m) => m.StockLevelsTab), { loading: PanelLoading });
const ItemCatalogueTab = dynamic(() => import("./InventoryTabPanels").then((m) => m.ItemCatalogueTab), { loading: PanelLoading });
const StoresTab = dynamic(() => import("./InventoryTabPanels").then((m) => m.StoresTab), { loading: PanelLoading });
const MovementsTab = dynamic(() => import("./InventoryTabPanels").then((m) => m.MovementsTab), { loading: PanelLoading });
...
export function tx(v: unknown, fallback = "\u2014") {
...
export function normalizeInventoryCategory(value: unknown, fallback = "") {
...
export default function InventoryPage() {
  return <InventoryTabPage initialTab="stock" />;
}

/** Shared Inventory workspace, rendered by a real route per tab (see the
 * sibling folders here) instead of the old inventory/[tab] -> redirect() ->
 * ?tab= shim. */
...
function InventoryWorkspace({ initialTab }: { initialTab: ActiveTab }) {
    ...
    <main className="min-h-full bg-ink p-4 text-paper sm:p-6">
      ...
      {tab === "catalogue" && (
        <ItemCatalogueTab
          ...
          itemTypeFilter={itemTypeFilter} setItemTypeFilter={setItemTypeFilter}
          setShowAddItem={setShowAddItem} loading={loading} catalogue={catalogue}
          filteredCatalogue={filteredCatalogue} setCatalogueDetail={setCatalogueDetail}
      ...
      {tab === "movements" && (
        <MovementsTab
          movDateFrom={movDateFrom} setMovDateFrom={setMovDateFrom}
          movDateTo={movDateTo} setMovDateTo={setMovDateTo}
          movTypeFilter={movTypeFilter} setMovTypeFilter={setMovTypeFilter}
          movStoreFilter={movStoreFilter} setMovStoreFilter={setMovStoreFilter}
          contextualStores={contextualStores} loading={loading} movements={movements}
...
export function MovBadge({ type }: { type: string }) {
