...
export function AEGISShowcase() {
  ...
  return (
    <section className="py-28 md:py-44 bg-void border-y border-ink-mid overflow-hidden">
      <div className="container max-w-[1400px] mx-auto px-6 md:px-10 lg:px-16 xl:px-20">
        <RevealOnScroll>
          <div className="max-w-3xl mb-16">
            <SectionLabel label="Platform" />
            <h2 className="text-headline-xl text-paper mt-4 mb-6">Engineered Operations.<br className="hidden md:block"/> Complete Visibility.</h2>
            <p className="text-body-lg text-slate-light leading-relaxed max-w-2xl">
        ...
        <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-16">
          {modules.map((mod, i) => (
            <StaggerItem key={i}>
              <div className="group p-4 rounded-sm border border-ink-mid bg-ink flex flex-col items-center justify-center text-center hover:border-info hover:bg-info-ghost transition-all cursor-pointer h-full">
                <mod.icon className="w-6 h-6 text-slate group-hover:text-info transition-colors mb-3" strokeWidth={1.5} />
                <span className="font-sans text-[10px] font-semibold uppercase tracking-widest text-slate-light group-hover:text-paper transition-colors">{mod.label}</span>
              </div>
            </StaggerItem>
          ))}
        </StaggerContainer>
        ...
        <RevealOnScroll delay={0.4} className="text-center">
          <Link href="/login">
            <Button variant="ghostWhite" className="border-info/50 text-info hover:border-info hover:bg-info-ghost">
