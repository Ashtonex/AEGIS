...
type TourPosition = {
  top: number;
  left: number;
  placement: NonNullable<ModuleTourStep["placement"]>;
  highlight?: DOMRect;
};
...
// A per-module counterpart to DashboardTour - same highlight/portal/step
// mechanics, but driven by a `steps` prop instead of one hardcoded array,
// since every module needs its own real content and its own target
// elements. Kept as a sibling file rather than folding into DashboardTour
// itself so the existing global-tour contract test (which asserts literal
// STEPS-based JSX text in that file) has nothing to break.
export function ModuleTour({
  ...
  const step = useMemo(() => steps[stepIndex] ?? steps[0], [steps, stepIndex]);
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = () => {
      ...
      let resolvedPlacement: NonNullable<ModuleTourStep["placement"]> = placement;
